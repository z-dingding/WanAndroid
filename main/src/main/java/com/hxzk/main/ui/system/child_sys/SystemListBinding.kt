...
fun setItems(listView: RecyclerView, items: List<SystemModel>?) {
    items?.let {
        //submitList()更新现有列表
        (listView.adapter as SystemItemAdapter).submitList(items)
    }
}
...
fun setFlexItems(flexContainer: FlexboxLayout, items: List<Children>?,listener: FlexItemClickListener) {
    items?.let {
...
private fun createBaseFlexItemTextView(context: Context, index: Int, items: List<Children>): TextView {
